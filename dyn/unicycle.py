...
class Unicycle(Model):
    def __init__(self, E_func=None):
        ...
        x_max = np.array([1000, 1000, 2*np.pi, 2.2]) # x, y, theta, v
        x_min = np.array([-1000, -1000, -2*np.pi, -1.])
        ...
        self.Gheading = np.array([[0,0,1,0,0,0]])
        self.Gheadingf = np.array([[0,0,1,0]])
    ...
    def ode(self, X, u):
        ...
        thetadot = u[0]
        vdot = u[1]
        Xd = ca.vertcat(xdot, ydot, thetadot, vdot)
        return Xd
