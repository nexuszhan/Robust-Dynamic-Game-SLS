...
# We use a LQR cost
Q = [Diagonal(ones(SVector{model.ni[i],T})) for i=1:p] # Quadratic state cost
R = [Diagonal(ones(SVector{model.mi[i],T})) for i=1:p] # Quadratic control cost
# Desrired state
xf = [SVector{model.ni[1],T}([2.5, 0.1, 0, 0]),
      SVector{model.ni[2],T}([-2.5, -0.1, 3.14159, 0]),
      ]
# Desired control
...
function plot_traj_with_constraints(prob, xc, yc, r; kwargs...)
    ...
    ri = [0.1 for j=1:N]
    plot_circles!(traj, xi, yi, ri; fillalpha=0.9, fillcolor=:red, linecolor=:red, linestyle=:solid
    )
    xi = [Algames.state(prob.pdtraj.pr[k])[model.pz[2][1]] for k=1:N] 
    yi = [Algames.state(prob.pdtraj.pr[k])[model.pz[2][2]] for k=1:N]
    ri = [0.1 for j=1:N]
    plot_circles!(traj, xi, yi, ri; fillalpha=0.9, fillcolor=:blue, linecolor=:blue, linestyle=:solid)

    plot_circles!(traj, xc, yc, r; kwargs...)
...
writedlm("Algame_narrow_corridor_traj.csv", X, ',')
