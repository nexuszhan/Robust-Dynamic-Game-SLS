...
def generate_initial_guess(planner:IBR, initial_guess_file:str):
    ...
    for idx in range(planner.N_agent):
        if idx % 3 != 0:
            ...
            # solution = solver.solve(x0, goal, planner.static_obstacles, [], planner.max_dists[idx], planner.min_dists[idx], leaders, [], half_cones[idx], planner.ca_weight, planner.prox_weight, planner.use_LQR[idx])
